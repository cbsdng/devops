...
if [ "${REGGAE}" = "yes" ]; then
  ...
  sudo tmux new-session -s "${backend_app_name}" -d "make -C services/terminal devel"
  ...
  sudo tmux split-window -v -p 50 -t 0 "${BIN_DIR}/terminal.sh"
  sudo tmux split-window -v -p 50 -t 2 "make -C services/backend celery"
  ...
  tmux new-session -s "${backend_app_name}" -d "env OFFLINE=${OFFLINE} SYSPKG=${SYSPKG} ${PROJECT_ROOT}/services/terminal/bin/devel.sh"
  ...
  tmux split-window -v -p 50 -t 0 "${BIN_DIR}/terminal.sh"
  tmux split-window -v -p 50 -t 2 "${BIN_DIR}/workers.sh"
